...
import { KIOSK_MODE_CLASS, PLAYER_VIEWPORT_CHANGE_EVENT } from '../utils/playerViewport';
...
export function PlayerShell({ isLoading, error, warning, children }: PlayerShellProps) {
  ...
  useEffect(() => {
    document.body.classList.toggle(KIOSK_MODE_CLASS, isKiosk);
    window.dispatchEvent(new Event(PLAYER_VIEWPORT_CHANGE_EVENT));
    return () => document.body.classList.remove(KIOSK_MODE_CLASS);
  }, [isKiosk]);
  ...
  return (
    <div className='w-full h-full bg-black flex flex-col lg:flex-row overflow-hidden'>
      <div
        className={`relative flex-1 min-w-0 min-h-0 bg-black flex flex-col items-center justify-center overflow-hidden${isKiosk ? ' cursor-none select-none' : ' group'}`}
      ...
      {!isKiosk && (
        <aside className='flex-shrink-0 h-[90px] lg:h-full lg:w-[160px]' aria-label='Advertisement'>
